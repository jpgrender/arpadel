...
import {
  ...
  setDoc, deleteDoc, addDoc, serverTimestamp, writeBatch,
...
export async function saveProfile(profile) {
  await setDoc(doc(db, "profiles", String(profile.id)), profile, { merge: true });
}
export async function uploadProfilePhoto(playerId, base64) {
  // base64 already cropped and converted — store directly in Firestore
  await setDoc(doc(db, "profiles", String(playerId)), { photoURL: base64 }, { merge: true });
  return base64;
}

export async function saveTournament(tournament) {
  const id = tournament.id || String(Date.now());
  await setDoc(doc(db, "tournaments", id), { ...tournament, id });
  return id;
}
export async function removeTournament(id) {
  await deleteDoc(doc(db, "tournaments", String(id)));
}
export async function addTournamentMatch(tournamentId, entry) {
  await addDoc(collection(db, "tournaments", String(tournamentId), "matches"), {
...
export async function clearPairHistory() {
  await setDoc(doc(db, "config", "pairHistory"), {});
}
export async function clearSession() {
  await setDoc(doc(db, "config", "session"), {
    active: false, courts: 2, mode: "nivelado", matchType: "short",
    attending: [], matches: [], waitingPair: [], rotations: [], rotationStep: 0, sittingOut: [],
  });
}
export async function clearPlayerStats(players) {
  const batch = writeBatch(db);
  for (const p of players) {
    batch.set(doc(db, "players", String(p.id)), { ...p, pts: 0, wins: 0, matches: 0 });
  }
  await batch.commit();
}
