...
// ── PlayerModal ──────────────────────────────────────────────────────────────
export function PlayerModal({ player, profile, user, onSave, onSaveProfile, onSaveUser, onDelete, onClose }) {
  const isNew = !player.id;
  const [tab, setTab] = useState('stats');
  const [confirmDelete, setConfirmDelete] = useState(false);
  const [form, setForm] = useState({ name: player.name ?? '', pts: player.pts ?? 150, wins: player.wins ?? 0, matches: player.matches ?? 0, surveyPts: player.surveyPts ?? 0 });
  const set = (k, v) => setForm(f => ({ ...f, [k]: v }));
  const [pform, setPform] = useState({ fullName: profile?.fullName ?? '', birthdate: profile?.birthdate ?? '', side: profile?.side ?? '', hand: profile?.hand ?? '', photoURL: profile?.photoURL ?? '' });
  const setp = (k, v) => setPform(f => ({ ...f, [k]: v }));
  const [uform, setUform] = useState({ pin: user?.pin ?? '', role: user?.role ?? 'user' });
  const setu = (k, v) => setUform(f => ({ ...f, [k]: v }));

  function handleSaveAll() {
    if (!form.name.trim()) return;
    onSave({ ...player, ...form, pts: Number(form.pts), wins: Number(form.wins), matches: Number(form.matches), surveyPts: Number(form.surveyPts) || 0 });
    if (onSaveProfile) onSaveProfile({ id: player.id, ...pform });
    if (onSaveUser && user) onSaveUser({ ...user, ...uform });
    onClose();
  }

  const tabs = [{ id: 'stats', label: '📊 Stats' }, { id: 'profile', label: '👤 Perfil' }, { id: 'user', label: '🔐 Acceso' }];
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: '#000000cc', display: 'flex', alignItems: 'flex-end', justifyContent: 'center', zIndex: 200 }} onClick={onClose}>
      <div style={{ background: '#181828', borderRadius: '20px 20px 0 0', padding: '20px 20px 40px', width: '100%', maxWidth: 440, border: '1px solid #ffffff15', maxHeight: '92vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
        <div style={{ width: 40, height: 4, background: '#ffffff20', borderRadius: 2, margin: '0 auto 16px' }} />
        <div style={{ fontSize: 16, fontWeight: 900, color: '#fff', marginBottom: 16 }}>{isNew ? '➕ Agregar jugador' : `✏️ ${player.name}`}</div>

        {!isNew && (
          <div style={{ display: 'flex', gap: 6, marginBottom: 20, background: '#ffffff08', borderRadius: 12, padding: 4 }}>
            {tabs.map(t => (
              <button key={t.id} onClick={() => setTab(t.id)}
                style={{ flex: 1, padding: '9px 4px', borderRadius: 9, border: 'none', background: tab === t.id ? '#ffffff15' : 'transparent', color: tab === t.id ? '#fff' : '#555', fontWeight: 700, fontSize: 12, cursor: 'pointer' }}>
                {t.label}
              </button>
            ))}
          </div>
        ...
        {(tab === 'stats' || isNew) && (
          <>
            ...
            <div style={{ marginBottom: 20, background: '#6ab4ff08', border: '1px solid #6ab4ff22', borderRadius: 10, padding: '12px' }}>
              <div style={{ fontSize: 11, color: '#6ab4ff', fontWeight: 700, letterSpacing: 1, marginBottom: 4 }}>PUNTAJE ENCUESTA</div>
              <div style={{ fontSize: 11, color: '#555', marginBottom: 8 }}>Solo visible para Admin. Se usa para calcular el nivel real en el sorteo de parejas.</div>
              <input type='number' value={form.surveyPts} onChange={e => set('surveyPts', e.target.value)} style={inputStyle} placeholder='0' />
            </div>
          </>
        )}
        ...
        {tab === 'profile' && !isNew && (
          <>
            <div style={{ marginBottom: 16, display: 'flex', alignItems: 'center', gap: 14 }}>
              {pform.photoURL
                ? <img src={pform.photoURL} style={{ width: 64, height: 64, borderRadius: '50%', objectFit: 'cover', border: '2px solid #00d4aa' }} />
                : <div style={{ width: 64, height: 64, borderRadius: '50%', background: '#00d4aa22', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22, color: '#00d4aa', fontWeight: 900 }}>{player.name[0]}</div>
              }
              <div style={{ flex: 1 }}>
                {pform.photoURL && (
                  <button onClick={() => setp('photoURL', '')}
                    style={{ background: '#ff6b6b15', border: '1px solid #ff6b6b33', borderRadius: 8, padding: '7px 14px', color: '#ff6b6b', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>
                    🗑 Eliminar foto
                  </button>
                )}
                {!pform.photoURL && <div style={{ fontSize: 12, color: '#444' }}>Sin foto cargada</div>}
              </div>
            </div>
            <div style={{ marginBottom: 14 }}>
              <div style={{ fontSize: 11, color: '#888', fontWeight: 700, letterSpacing: 1, marginBottom: 6 }}>NOMBRE COMPLETO</div>
              <input value={pform.fullName} onChange={e => setp('fullName', e.target.value)} placeholder='Nombre completo' style={inputStyle} />
            </div>
            <div style={{ marginBottom: 14 }}>
              <div style={{ fontSize: 11, color: '#888', fontWeight: 700, letterSpacing: 1, marginBottom: 6 }}>FECHA DE NACIMIENTO</div>
              <input type='date' value={pform.birthdate} onChange={e => setp('birthdate', e.target.value)} style={inputStyle} />
            </div>
            <div style={{ marginBottom: 14 }}>
              <div style={{ fontSize: 11, color: '#888', fontWeight: 700, letterSpacing: 1, marginBottom: 8 }}>LADO DE LA CANCHA</div>
              <div style={{ display: 'flex', gap: 8 }}>
                {['Drive','Revés','Ambos'].map(v => (
                  <div key={v} onClick={() => setp('side', v)} style={{ flex: 1, padding: '10px', borderRadius: 10, textAlign: 'center', cursor: 'pointer', fontSize: 13, fontWeight: 700, background: pform.side === v ? '#00d4aa22' : '#ffffff08', border: pform.side === v ? '2px solid #00d4aa' : '2px solid transparent', color: pform.side === v ? '#00d4aa' : '#555' }}>{v}</div>
                ))}
              </div>
            </div>
            <div style={{ marginBottom: 20 }}>
              <div style={{ fontSize: 11, color: '#888', fontWeight: 700, letterSpacing: 1, marginBottom: 8 }}>MANO HÁBIL</div>
              <div style={{ display: 'flex', gap: 8 }}>
                {['Diestro','Zurdo'].map(v => (
                  <div key={v} onClick={() => setp('hand', v)} style={{ flex: 1, padding: '10px', borderRadius: 10, textAlign: 'center', cursor: 'pointer', fontSize: 13, fontWeight: 700, background: pform.hand === v ? '#6ab4ff22' : '#ffffff08', border: pform.hand === v ? '2px solid #6ab4ff' : '2px solid transparent', color: pform.hand === v ? '#6ab4ff' : '#555' }}>{v}</div>
                ))}
              </div>
            </div>
          </>
        ...
        <button onClick={handleSaveAll} style={{ width: '100%', background: 'linear-gradient(135deg,#00d4aa,#0066ff)', border: 'none', borderRadius: 12, padding: '14px', color: '#fff', fontWeight: 800, fontSize: 15, cursor: 'pointer', marginBottom: 10 }}>
          {isNew ? 'Agregar al grupo' : 'Guardar cambios'}
        </button>
        {!isNew && !confirmDelete && (
          <button onClick={() => setConfirmDelete(true)}
            style={{ width: '100%', background: 'transparent', border: '1px solid #ff6b6b44', borderRadius: 12, padding: '13px', color: '#ff6b6b', fontWeight: 700, fontSize: 14, cursor: 'pointer' }}>
            🗑 Eliminar jugador
          </button>
        )}
        {!isNew && confirmDelete && (
          <div style={{ background: '#ff6b6b11', border: '1px solid #ff6b6b44', borderRadius: 14, padding: '16px' }}>
            <div style={{ fontSize: 14, fontWeight: 800, color: '#ff6b6b', marginBottom: 6, textAlign: 'center' }}>
              ⚠️ ¿Eliminar a {player.name}?
            </div>
            <div style={{ fontSize: 12, color: '#888', marginBottom: 14, textAlign: 'center' }}>
              Se borran todos sus datos y estadísticas. No se puede deshacer.
            </div>
            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={() => setConfirmDelete(false)}
                style={{ flex: 1, background: 'transparent', border: '1px solid #ffffff20', borderRadius: 10, padding: '12px', color: '#aaa', fontWeight: 700, cursor: 'pointer' }}>
                Cancelar
              </button>
              <button onClick={() => onDelete(player.id)}
                style={{ flex: 2, background: '#ff6b6b', border: 'none', borderRadius: 10, padding: '12px', color: '#fff', fontWeight: 800, fontSize: 14, cursor: 'pointer' }}>
                Sí, eliminar
              </button>
            </div>
          </div>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div style={{ fontSize: 13, color: "#ccc", marginBottom: 20 }}>Cambiá roles y PINs.</div>
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "#000000cc", zIndex: 200, display: "flex", alignItems: "flex-end", justifyContent: "center" }}>
      <div style={{ background: "#13131f", borderRadius: "20px 20px 0 0", padding: "24px 20px 40px", width: "100%", maxWidth: 420 }}>
        ...
        <div style={{ fontSize: 13, color: "#ccc", marginBottom: 24 }}>Cambiá tu PIN · {isPrivileged ? "6 dígitos" : "4 dígitos"}</div>

        <div style={{ marginBottom: 14 }}>
          <div style={{ fontSize: 13, color: "#ccc", marginBottom: 6, fontWeight: 600 }}>PIN ACTUAL</div>
        ...
        <div style={{ marginBottom: 14 }}>
          <div style={{ fontSize: 13, color: "#ccc", marginBottom: 6, fontWeight: 600 }}>PIN NUEVO</div>
          <div style={{ position: "relative" }}>
        ...
        <div style={{ marginBottom: 20 }}>
          <div style={{ fontSize: 13, color: "#ccc", marginBottom: 6, fontWeight: 600 }}>CONFIRMAR PIN NUEVO</div>
      ...
      <div style={{ background: "#13131f", borderRadius: "20px 20px 0 0", padding: "24px 20px 40px", width: "100%", maxWidth: 420, border: "1px solid #ffffff15", maxHeight: "88vh", overflowY: "auto" }}
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", marginBottom: 20 }}>
          ...
          {profile?.fullName && profile.fullName !== player.name && (
            <div style={{ fontSize: 13, color: "#ccc", marginTop: 2 }}>{profile.fullName}</div>
          )}
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0d0d1a", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "0 24px" }}>
      ...
      <div style={{ width: "100%", maxWidth: 380, background: "#181828", borderRadius: 20, padding: "28px 24px", border: "1px solid #ffffff10" }}>
        ...
        <div style={{ fontSize: 13, color: "#ccc", marginBottom: 24 }}>Seleccioná tu nombre e ingresá tu PIN</div>

        <div style={{ marginBottom: 16 }}>
          <div style={{ fontSize: 12, color: "#bbb", fontWeight: 700, letterSpacing: 1, marginBottom: 8 }}>¿QUIÉN SOS?</div>
        ...
        {selected && (
          <div style={{ marginBottom: 20 }}>
            <div style={{ fontSize: 12, color: "#bbb", fontWeight: 700, letterSpacing: 1, marginBottom: 8 }}>PIN</div>
            <div style={{ position: "relative" }}>
        ...
        <div style={{ fontSize: 13, color: "#ccc", textAlign: "center", marginTop: 16 }}>¿No tenés PIN? Pedíselo al admin del grupo.</div>
      </div>
      <div style={{ marginTop: 28, textAlign: "center" }}>
        <div style={{ fontSize: 12, color: "#bbb", fontWeight: 600 }}>Desarrollado por Juan Pablo Gómez</div>
        <a href="mailto:argentinapadel@gmail.com" style={{ fontSize: 12, color: "#bbb", textDecoration: "none" }}>
          argentinapadel@gmail.com
        </a>
        <div style={{ fontSize: 12, color: "#bbb", marginTop: 2 }}>© 2026 ARPadel · Todos los derechos reservados</div>
      </div>
    </div>
  );
}
...
export function FreeMatchModal({ players, tournaments, busyPlayerIds = [], onSave, onClose }) {
  const [step,      setStep]      = useState(0); // 0=tipo, 1=pareja1, 2=pareja2, 3=confirm
  ...
  const busyOrSelected = id => busyPlayerIds.includes(id) || team1.some(p => p.id === id) || team2.some(p => p.id === id);

  function selectPlayer(p, teamNum) {
    if (teamNum === 1) {
      if (team1.some(x => x.id === p.id)) return;
      if (team1.length < 2) { setTeam1(t => [...t, p]); if (team1.length === 1) setStep(2); }
    } else {
      if (team2.some(x => x.id === p.id)) return;
      if (team2.length < 2) { setTeam2(t => [...t, p]); if (team2.length === 1) setStep(3); }
    }
  }
  function removePlayer(id, teamNum) {
    if (teamNum === 1) { setTeam1(t => t.filter(x => x.id !== id)); setStep(1); }
    else { setTeam2(t => t.filter(x => x.id !== id)); setStep(2); }
  }
  function handleSave() {
    if (team1.length !== 2 || team2.length !== 2) return;
    onSave({ team1, team2, matchType, tournamentId: tournId || null });
    setSaved(s => s + 1);
    setTeam1([]); setTeam2([]); setTournId(''); setStep(1);
  }

  const sortedPlayers = [...players].sort((a, b) => a.name.localeCompare(b.name));
  const teamColor1 = '#00d4aa';
  const teamColor2 = '#6ab4ff';

  const PlayerList = ({ teamNum }) => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      {sortedPlayers.map(p => {
        const inTeam = (teamNum === 1 ? team1 : team2).some(x => x.id === p.id);
        const busy = !inTeam && busyOrSelected(p.id);
        const color = teamNum === 1 ? teamColor1 : teamColor2;
        return (
          <div key={p.id} onClick={() => !busy && selectPlayer(p, teamNum)}
            style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 14px', borderRadius: 12, cursor: busy ? 'default' : 'pointer',
              background: inTeam ? `${color}22` : busy ? '#ffffff05' : '#ffffff0d',
              border: inTeam ? `2px solid ${color}` : busy ? '1px solid #ffffff08' : '1px solid #ffffff15',
              opacity: busy ? 0.4 : 1 }}>
            <Avatar name={p.name} pts={p.pts} size={34} />
            <span style={{ flex: 1, fontSize: 15, fontWeight: 700, color: busy ? '#666' : '#fff' }}>{p.name}{busy ? '  🔒' : ''}</span>
            <LevelBadge pts={p.pts} />
            {inTeam && <span style={{ fontSize: 13, color, fontWeight: 800 }}>✓</span>}
          </div>
        );
      })}
    </div>
  ...
  const TeamSlot = ({ team, teamNum, label }) => {
    const color = teamNum === 1 ? teamColor1 : teamColor2;
    return (
      <div style={{ flex: 1, background: `${color}10`, border: `2px solid ${team.length === 2 ? color : color + '30'}`, borderRadius: 12, padding: '12px', minHeight: 70 }}>
        <div style={{ fontSize: 11, color, fontWeight: 800, marginBottom: 8, letterSpacing: 1 }}>{label}</div>
        {team.map(p => (
          <div key={p.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
            <span style={{ fontSize: 14, color: '#fff', fontWeight: 700 }}>{p.name}</span>
            <button onClick={() => removePlayer(p.id, teamNum)}
              style={{ background: 'transparent', border: 'none', color: '#ff6b6b', cursor: 'pointer', fontSize: 16, lineHeight: 1 }}>✕</button>
          </div>
        ))}
        {team.length < 2 && (
          <div style={{ fontSize: 12, color: color + '99', fontStyle: 'italic' }}>
            {team.length === 0 ? 'Elegí 2 jugadores' : 'Elegí 1 más'}
          </div>
        )}
      </div>
    );
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: '#000000cc', zIndex: 200, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }} onClick={onClose}>
      <div style={{ background: '#13131f', borderRadius: '20px 20px 0 0', padding: '20px 20px 40px', width: '100%', maxWidth: 440, border: '1px solid #ffffff15', maxHeight: '92vh', overflowY: 'auto' }}
        onClick={e => e.stopPropagation()}>
        <div style={{ width: 40, height: 4, background: '#ffffff30', borderRadius: 2, margin: '0 auto 16px' }} />

        {/* Header */}
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          <div style={{ fontSize: 18, fontWeight: 900, color: '#fff' }}>⚡ Nuevo partido</div>
          {saved > 0 && <div style={{ fontSize: 13, color: '#00d4aa', fontWeight: 800 }}>{saved} agregado{saved !== 1 ? 's' : ''} ✓</div>}
        </div>
        ...
        {/* Resumen de equipos */}
        <div style={{ display: 'flex', gap: 10, marginBottom: 20 }}>
          <TeamSlot team={team1} teamNum={1} label="PAREJA 1" />
          <TeamSlot team={team2} teamNum={2} label="PAREJA 2" />
        </div>

        {/* Selector de jugadores */}
        {(step === 1 || step === 0) && (
          <>
            <div style={{ fontSize: 13, fontWeight: 800, color: teamColor1, marginBottom: 10 }}>
              👇 Elegí los jugadores de la PAREJA 1
            </div>
            <PlayerList teamNum={1} />
          </>
        )}
        {(step === 2) && team1.length === 2 && (
          <>
            <div style={{ fontSize: 13, fontWeight: 800, color: teamColor2, marginBottom: 10 }}>
              👇 Elegí los jugadores de la PAREJA 2
            </div>
            <PlayerList teamNum={2} />
          </>
        )}
        {(step === 3 || (team1.length === 2 && team2.length === 2)) && (
          <>
            {activeTournaments.length > 0 && (
              <div style={{ marginBottom: 16 }}>
                <div style={{ fontSize: 12, color: '#fff', fontWeight: 800, letterSpacing: 1, marginBottom: 8 }}>TORNEO (opcional)</div>
                <select value={tournId} onChange={e => setTournId(e.target.value)}
                  style={{ width: '100%', background: '#0d0d1a', border: '1px solid #ffffff30', borderRadius: 10, padding: '12px 14px', color: tournId ? '#fff' : '#888', fontSize: 14, outline: 'none' }}>
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
          ...
          <button onClick={handleSave} disabled={team1.length !== 2 || team2.length !== 2}
            style={{ flex: 2, background: team1.length === 2 && team2.length === 2 ? 'linear-gradient(135deg,#00d4aa,#0066ff)' : '#333', border: 'none', borderRadius: 12, padding: '14px', color: team1.length === 2 && team2.length === 2 ? '#fff' : '#666', fontWeight: 800, fontSize: 14, cursor: team1.length === 2 && team2.length === 2 ? 'pointer' : 'not-allowed' }}>
            ⚡ Agregar partido
          </button>
        </div>
      </div>
...
// ── AttendanceCard ────────────────────────────────────────────────────────────
export function AttendanceCard({ iAmGoing, count, onToggle }) {
  return (
    <div onClick={onToggle}
      ...
      {count > 0 && (
        <div style={{ textAlign: "center", flexShrink: 0 }}>
          <div style={{ fontSize: 22, fontWeight: 900, color: "#fff" }}>{count}</div>
...
// ── CloseJornadaModal ────────────────────────────────────────────────────────
export function CloseJornadaModal({ matches, onConfirm, onClose }) {
          ...
          <>
            ...
            {openMatches.map(m => (
              <div key={m.id} style={{ background: "#ffffff08", border: "1px solid #ffffff15", borderRadius: 10, padding: "12px 14px", marginBottom: 8 }}>
                <div style={{ fontSize: 13, fontWeight: 700, color: "#fff" }}>{m.team1.map(p => p.name).join(" & ")}</div>
                <div style={{ fontSize: 12, color: "#888", margin: "2px 0" }}>vs</div>
                <div style={{ fontSize: 13, fontWeight: 700, color: "#fff" }}>{m.team2.map(p => p.name).join(" & ")}</div>
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "#000000cc", zIndex: 200, display: "flex", alignItems: "flex-end", justifyContent: "center" }}
      ...
      <div style={{ background: "#13131f", borderRadius: "20px 20px 0 0", padding: "20px 20px 40px", width: "100%", maxWidth: 420, border: "1px solid #ffffff15", maxHeight: "90vh", overflowY: "auto" }}
        ...
        <div style={{ fontSize: 13, color: "#ccc", marginBottom: 20 }}>Cada acción tiene doble confirmación. No se pueden deshacer.</div>
