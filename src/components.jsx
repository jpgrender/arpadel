        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", marginBottom: 20 }}>
          {profile?.photoURL ? (
            <img src={profile.photoURL} alt={player.name}
              style={{ width: 88, height: 88, borderRadius: "50%", objectFit: "cover", border: "3px solid #00d4aa", marginBottom: 10 }} />
          ) : (
...
// ── FreeMatchModal ────────────────────────────────────────────────────────────
export function FreeMatchModal({ players, tournaments, onSave, onClose }) {
  ...
  function handleSave() {
    ...
    setError("");
    onSave({ team1, team2, score1, score2, tournamentId: tournId || null });
  }
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "#000000cc", zIndex: 200, display: "flex", alignItems: "flex-end", justifyContent: "center" }} onClick={onClose}>
      <div style={{ background: "#13131f", borderRadius: "20px 20px 0 0", padding: "20px 20px 40px", width: "100%", maxWidth: 420, border: "1px solid #ffffff15", maxHeight: "92vh", overflowY: "auto" }}
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 16 }}>
          {available.sort((a,b) => a.name.localeCompare(b.name)).map(p => (
            <div key={p.id} style={{ display: "flex", gap: 4 }}>
              <div onClick={() => addToTeam(p, 1)} style={{ background: "#00d4aa22", border: "1px solid #00d4aa44", borderRadius: 8, padding: "5px 10px", fontSize: 12, color: "#00d4aa", cursor: team1.length < 2 ? "pointer" : "not-allowed", opacity: team1.length >= 2 ? 0.4 : 1, fontWeight: 700 }}>
                {p.name} →1
              </div>
              <div onClick={() => addToTeam(p, 2)} style={{ background: "#6ab4ff22", border: "1px solid #6ab4ff44", borderRadius: 8, padding: "5px 10px", fontSize: 12, color: "#6ab4ff", cursor: team2.length < 2 ? "pointer" : "not-allowed", opacity: team2.length >= 2 ? 0.4 : 1, fontWeight: 700 }}>
                →2
              </div>
            </div>
          ))}
        ...
        {/* Score */}
        <div style={{ fontSize: 11, color: "#aaa", fontWeight: 700, marginBottom: 8 }}>RESULTADO (games)</div>
