...
function CropModal({ imageSrc, onCrop, onCancel }) {
  ...
  React.useEffect(() => { draw(); }, [offset, scale]);
  ...
  function onDown(e) { e.preventDefault(); setDragging(true); setDragStart(getPos(e)); }
  function onMove(e) {
  ...
  function handleCrop() {
    ...
    const ctx = canvas.getContext("2d");
    const img = imgRef.current;
    const factor = 300 / SIZE;
    ctx.save();
    ctx.beginPath();
    ctx.arc(150, 150, 150, 0, Math.PI*2);
    ctx.clip();
    ctx.drawImage(img, offset.x * factor, offset.y * factor, img.width * scale * factor, img.height * scale * factor);
    ctx.restore();
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "#000000ee", zIndex: 300, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div style={{ background: "#13131f", borderRadius: 20, padding: 20, width: "100%", maxWidth: 360 }}>
        ...
        <div style={{ fontSize: 11, color: "#aaa", textAlign: "center", marginBottom: 16 }}>Arrastrá para mover · Scroll para zoom</div>
        ...
        <div style={{ marginBottom: 14 }}>
          <div style={{ fontSize: 10, color: "#aaa", marginBottom: 6, textAlign: "center" }}>ZOOM</div>
