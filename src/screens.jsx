...
function CropModal({ imageSrc, onCrop, onCancel }) {
  ...
  function onPointerMove(e) {
    ...
    e.preventDefault();
    const p = getXY(e);
    setOffset(o => ({ x: o.x + p.x - dragRef.current.lastX, y: o.y + p.y - dragRef.current.lastY }));
  ...
  function handleWheel(e) {
    e.preventDefault();
    setScale(s => Math.min(5, Math.max(initScale * 0.5, s - e.deltaY * 0.001)));
  }
  function handleCrop() {
      ...
      <div style={{ background: "#13131f", borderRadius: 20, padding: 20, width: "100%", maxWidth: 360 }}>
        ...
        <div style={{ marginBottom: 14 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
            <div style={{ fontSize: 10, color: "#aaa", fontWeight: 700 }}>ZOOM</div>
            <button onClick={resetZoom} style={{ fontSize: 11, color: "#6ab4ff", background: "transparent", border: "none", cursor: "pointer", fontWeight: 700 }}>↺ Reset</button>
          </div>
          <input type="range"
            min={String(initScale * 0.5)} max="4" step="0.01"
            value={scale}
